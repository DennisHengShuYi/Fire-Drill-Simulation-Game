...
func _on_body_entered(body):
	if body.has_method("process_smoke_inhalation") and "in_smoke_zone" in body:
		tracked_player = body
		var active_smokes = body.get_meta("active_smokes") if body.has_meta("active_smokes") else []
...
func _on_body_exited(body):
	if body == tracked_player:
		if is_instance_valid(tracked_player):
			var active_smokes = tracked_player.get_meta("active_smokes") if tracked_player.has_meta("active_smokes") else []
